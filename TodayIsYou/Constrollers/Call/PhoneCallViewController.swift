...
//        self.sendMessage("Room Out")
    ...
    ///Mark life cycle
    static func initWithType(_ type: ConnectionType, roomKey: String, toUserId: String) -> PhoneCallViewController {
...
//        self.configureVideoRenderer()
        ...
        lbTakMsg.text = TalkMemo.localizedString(toUser.contents)
        lbUserName.text = toUser.user_name
        lbGender.text = Gender.localizedString(toUser.user_sex)
        lbAge.text = Age.localizedString(toUser.user_age)
        lbGoodCnt.text = "\(toUser.good_cnt)".addComma()
    }
...
//        이후 10초당 100포인트 차감
    ...
    func stopTimer() {
        ...
        self.webRtcClient.close()
        self.requestPaymentEndPoint()
        appDelegate.showScoreAlert(toUserId: toUserId, toUserName: toUser.user_name)
    ...
    @IBAction func onClickedBtnActions(_ sender: UIButton) {
        if sender == btnSpeaker {
            ...
            CAlertViewController.show(type: .alert, title: nil, message: NSLocalizedString("activity_txt402", comment:"통화를 종료합니다"), actions: [.cancel, .ok]) { (vcs, selItem, action) in
                vcs.dismiss(animated: true)
                if action == 1 {
                    self.finishPhoneCall()
                    self.closeVC()
                }
            }
        }
    }
    private func finishPhoneCall() {
        var param:[String:Any] = [:]
        param["from_user_id"] =  ShareData.ins.myId
        param["to_user_id"] = self.toUserId
        param["msg"] = "CAM_NO"
