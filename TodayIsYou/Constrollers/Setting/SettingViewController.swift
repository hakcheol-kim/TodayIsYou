...
//            let vc = PhotoManagerViewController.instantiateFromStoryboard(.main)!
            if ShareData.ins.isReview {
                self.showToast(NSLocalizedString("activity_txt454", comment: "준비중인 서비스입니다."))
                return
            }
            ...
            let alert = CAlertViewController.init(type: .alert, title: NSLocalizedString("activity_txt495", comment: "신고하기"), message: nil, actions: [.cancel, .ok]) { (vcs, selItem, index) in
                ...
                if (index == 1) {
                    ...
                    ///- user_id:, to_user_id:, to_user_name, memo:
                    let param = ["user_name":"", "user_id":ShareData.ins.myId, "memo":reportWriteStr, "to_user_id" : "all_config", "to_user_name":""]
            ...
            alert.addTextView(NSLocalizedString("activity_txt496", comment: "신고대상"), UIEdgeInsets(top: 10, left: 8, bottom: 8, right: 8), 40)
