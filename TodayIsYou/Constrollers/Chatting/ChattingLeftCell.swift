...
class ChattingLeftCell: UITableViewCell {
    ...
    func configurationData(_ data: ChatMessage?, profile_name: String?, toUserName: String) {
        ...
        if let memo = chat.memo {
            ...
            else if memo.hasPrefix("@@##$$|") {
                var msg = memo.replacingOccurrences(of: "@@##$$|", with: "")
                msg = "🎁 \(msg)"
                attr = NSMutableAttributedString.init(string: msg)
            }
            else {
        ...
        ivBgView.backgroundColor = UIColor.clear
        ivBgView.tintColor = .appColor(.redLight)
        ...
        if let file_name = chat.file_name, file_name.isEmpty == false {
            ...
            else if let url = Utility.thumbnailUrl(chat.to_user_id, file_name) {
                thumbnailUrl = url
            }
