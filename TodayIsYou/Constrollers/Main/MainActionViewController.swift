...
//                customView.lbMsg1.text = NSLocalizedString("activity_txt215", comment: "대화 목록에 있는 상대는 신청이 무료입니다")
                customView.lbMsg1.text = NSLocalizedString("popup_point_notification2", comment: "대화목록에 있는 상대는 신청이 무료입니다.\n1분이 후 10초 마다 영상통화시 200 포인트 씩 차감됩니다.\n음성통화시 100포인트씩 차감됩니다.")
...
//                customView.lbMsg1.text = NSLocalizedString("activity_txt212", comment: "영상 음성 채팅 신청시") + " \(0)" + NSLocalizedString("activity_txt213", comment: "포인트가 차감 됩니다.")
                customView.lbMsg1.text = NSLocalizedString("popup_point_notification1", comment: "영상 채팅 신청 시 최초 1,200포인트 차감 된 후 1분이 후 10초 마다 200 포인트 씩 차감됩니다.\n음성통화시 최초 600포인트 차감 후 1분 이후 부터 100포인트씩 차감됩니다")
            }
        }
        else {
            customView.lbMsg2.isHidden = true
        ...
        if canCall {
            ...
            ApiManager.ins.requestPhoneCallInsertMsg(param: param) { (res) in
                ...
                if isSuccess == "01" {
                    ...
                    let vc = PhoneCallViewController.initWithType(.offer, roomKey: room_key, toUserId: to_user_id)
        ...
        if canCall {
            ...
            ApiManager.ins.requestCamCallInsertMsg(param: param) { (res) in
                ...
                if isSuccess == "01" {
                    ...
                    let vc = CamCallViewController.initWithType(.offer, roomKey: room_key, toUserId: to_user_id)
                    self.navigationController?.pushViewController(vc, animated: true)
                }
                else {
                    self.showErrorToast(res)
