...
class ApiManager: NSObject {
    ...
    func requestUer(param:[String:Any], success:ResSuccess?, failure:ResFailure?) {
        var param2 = param;
        param2["version_code"] = Bundle.main.appVersion.getNumberString()
        NetworkManager.ins.request(.post, "/api/talk/getUser.json",  clientPara(param2)) { (response) in
            success?(response)
        } failure: { (error) in
            failure?(error)
        }
    ...
    func requestServiceTerms(mode:String, success:ResSuccess?, failure:ResFailure?) {
        if ShareData.ins.serverLanguageCode == "kr" {
            ...
            let url = "\(baseUrl2)/app/yk/yk.php?mode=\(mode)"
    ...
    func requestSubscriptionTerm(success: ResSuccess?, failure:ResFailure?) {
        let url = baseUrl2+"/app/subs/subs_terms.php?language=\(ShareData.ins.serverLanguageCode)"
    ...
    func requestGetPoint(param:[String:Any],  success:ResSuccess?, failure:ResFailure?) {
        NetworkManager.ins.request(.post, "/api/talk/getPoint.json", clientPara(param), JSONEncoding.default, false) { (response) in
            success?(response)
        } failure: { (error) in
            failure?(error)
        }
    ...
    func requestSmsAuthCode(param:[String:Any], success:ResSuccess?, fail:ResFailure?) {
        NetworkManager.ins.request(.get, baseUrl2+"/app/sms/sms_send_todayisyou.php", param, URLEncoding.queryString) { (res) in
            success?(res)
        } failure: { (err) in
            fail?(err)
        }
    ...
    func requestRegistPartnerCode(param:[String:Any], succcess:ResSuccess?, fail:ResFailure?) {
        NetworkManager.ins.request(.get, baseUrl2+"/app/member/member_partner.php", param, URLEncoding.queryString) { (res) in
            succcess?(res)
        } failure: { (erro) in
            fail?(erro)
        }
    ...
    ///랜덤영상통화 신청
    func requestRandomCamCallInsertMsg(param:[String:Any], success:ResSuccess?, fail:ResFailure?) {
        NetworkManager.ins.request(.post, "/api/talk/insertVcRandomChatMsg.do", param, URLEncoding.queryString) { (res) in
            success?(res)
        } failure: { (error) in
            fail?(error)
        }
    ...
    //dbdbdeep 전송
    func requestdbdbdeep(param:[String:Any], success:ResSuccess?, fail:ResFailure?) {
        NetworkManager.ins.request(.get, "http://dbdbdeep.com/site19/gate/today/pay_result.php", param, URLEncoding.queryString) { res in
            success?(res)
        } failure: { error in
            fail?(error)
        }
    ...
    func requestPaymentInAppSubScription(param:[String:Any], success:ResSuccess?, fail:ResFailure?) {
        let url = baseUrl2+"/app/subs/subs_insert.php"
    ...
    func requestEventList(success: ResSuccess?, fail:ResFailure?) {
        NetworkManager.ins.request(.get, baseUrl2+"/app/banner/main_banner.php", nil) { res in
            success?(res)
        } failure: { error in
            fail?(error)
        }
...
//        http://dbdbdeep.com/site19/gate/today/join_result.php?dbdbdeep_userid=&dbdbdeep_tel=01031244920&referrer=TEST_S00259878ZC05487261&mb=Y
    ...
    func requestChangeAppType(type: String, userId:String, success:ResSuccess?, fail:ResFailure?) {
        let url = baseUrl2+"/app/check/app_type_check.php?app_type=\(type)&user_id=\(userId)"
        NetworkManager.ins.request(.get, url, nil) { res in
            success?(res)
        } failure: { error in
            fail?(error)
