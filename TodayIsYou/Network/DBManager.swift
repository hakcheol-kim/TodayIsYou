...
class DBManager: NSObject {
    ...
    func getChatMessagesWithUserId(userId:String, _ completion:((_ messages:[ChatMessage]?, _ error:Error?) -> Void)?) {
        let predicate = NSPredicate.init(format: "(%K = %@)", "from_user_id", userId)
        let des = NSSortDescriptor.init(key: "reg_date", ascending: true)
        let request = NSFetchRequest<NSFetchRequestResult>(entityName: DBName.chatmessage.rawValue)
        request.predicate = predicate
        request.sortDescriptors = [des]
    ...
    func deleteChatMessageWithUserId(userId:String, _ completion:((_ success:Bool, _ error:Error?) ->Void)?) {
        self.getChatMessagesWithUserId(userId: userId) { (chats, error) in
            guard let chats = chats, chats.isEmpty == false else {
                completion?(true, nil)
                return
            }
            for chat in chats {
                self.viewcontext.delete(chat)
                print("delete chat from_user_id: \(String(describing: chat.from_user_id))")
            }
